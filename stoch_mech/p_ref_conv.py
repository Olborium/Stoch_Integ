...
###########################
#
# The script reads M groups of trajectories computed using M reference Brownian processes with small time step dt.
# Each group contains N trajectories computed with N different time steps DT >= dt.
# The reference trajectory with DT = dt is computed using the 1st strong order method.
...
# The data points dist_av(DT) are fitted by a line (slope a, intercept b) in the log-log scale and in the region where the error is not dominated by the reference solution.
...
path = 'Out/Pendulum/RefConv'
files = os.listdir(path)
Qfin = np.zeros((N,M))
Pfin = np.zeros((N,M))
str = f'{Ns[0]:.0f}'+'_'+f'{BETA0}'+'_'+f'{GAMMA0}'
j = 0
for filename in files:
    if re.match('pendulum_qt_'+str+'_'+f'{j:.0f}',filename):
        filepath = os.path.join(path, filename)
        Qfin[0,j] = np.loadtxt(filepath)[-1] 
        j += 1
j = 0
for filename in files:        
    if re.match('pendulum_pt_'+str+'_'+f'{j:.0f}',filename):
        filepath = os.path.join(path, filename)
        Pfin[0,j] = np.loadtxt(filepath)[-1] 
        j += 1
for filename in files:
    for j in range(M):
        for i in range(1,N):
...
iMax = np.where(dist_av<0.05)[0][-1]+1
params, cov = curve_fit(f, np.log10(DTs[9:iMax+1]), np.log10(dist_av[8:iMax]))
(a, b) = (params[0], params[1])
print(a,b)

t = np.linspace(DTs[1],DTs[-1],100)
str = f'{BETA}'+f'{GAMMA}'+'_M='+f'{M:.0f}'+'_dt='+f'{DTs[0]:.5f}'+'_eta='+f'{ETA:.2f}'+'_int_'+INTEG
if b>0:
...
plt.savefig('Plots/pendulum_conv_ref_'+str+'.pdf', bbox_inches='tight')
